//#region node_modules/artplayer/dist/artplayer.mjs
/*!
* artplayer.js v5.4.0
...
var validator = /* @__PURE__ */ getDefaultExportFromCjs(requireOptionValidator());
var version$1 = "5.4.0";
var config$1 = {
	properties: [
		"audioTracks",
...
var userAgent = globalThis?.CUSTOM_USER_AGENT ?? (typeof navigator !== "undefined" ? navigator.userAgent : "");
var isSafari = /^(?:(?!chrome|android).)*safari/i.test(userAgent);
var isIOS = /iPad|iPhone|iPod/i.test(userAgent) && !window.MSStream;
var isIOS13 = isIOS || userAgent.includes("Macintosh") && navigator.maxTouchPoints >= 1;
var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent) || isIOS13;
			...
			if (!scale || scale === 1) resolve(img);
				...
				canvas.toBlob((blob) => {
					...
					scaledImg.onerror = function() {
						...
						reject(/* @__PURE__ */ new Error(`Image load failed: ${url}`));
					};
...
function getComposedPath(event) {
	...
	if (!path.includes(window) && window !== void 0) path.push(window);
	return path;
}
var ArtPlayerError = class extends Error {
...
function assToVtt(ass) {
	const reAss = /* @__PURE__ */ new RegExp("Dialogue:\\s\\d,(\\d+:\\d\\d:\\d\\d.\\d\\d),(\\d+:\\d\\d:\\d\\d.\\d\\d),([^,]*),([^,]*),(?:[^,]*,){4}([\\s\\S]*)$", "i");
...
function validElement(value, type, paths) {
	return errorHandle(type === s || type === n || value instanceof Element, `${paths.join(".")} require '${s}' or 'Element' type`);
}
var ComponentOption = {
	html: validElement,
	disable: `?${b}`,
...
var scheme = {
	...
	controls: [{
		...
		position: (value, _, paths) => {
			...
			return errorHandle(position.includes(value), `${paths.join(".")} only accept ${position.toString()} as parameters`);
		}
	}],
	quality: [{
		default: `?${b}`,
...
var Component = class {
	...
	set show(value) {
		...
		if (value) addClass($player, className);
	...
	add(getOption) {
		...
		else append(this.$parent, $ref);
		if (option.html) append($ref, option.html);
		if (option.style) setStyles($ref, option.style);
		if (option.tooltip) tooltip($ref, option.tooltip);
		const events = [];
		if (option.click) {
		...
		if (option.selector && ["left", "right"].includes(option.position)) this.selector(option, $ref, events);
		this[name] = $ref;
		this.cache.set(name, {
			$ref,
			events,
		...
		return {
			...
			mounted: ($panel) => {
				...
				art.on("video:ratechange", () => {
					const $current = queryAll("span", $panel).find((item) => Number(item.dataset.value) === art.playbackRate);
					if ($current) inverseClass($current, "art-current");
				});
			}
		};
...
var Contextmenu = class extends Component {
	...
	init() {
		...
		if (option.playbackRate) this.add(playbackRate$2({
			name: "playbackRate",
			index: 10
		}));
		if (option.aspectRatio) this.add(aspectRatio$2({
			name: "aspectRatio",
			index: 20
		}));
		if (option.flip) this.add(flip$2({
			name: "flip",
			index: 30
		}));
		this.add(info({
		...
		proxy($player, "click", (event) => {
			if (!includeFromEvent(event, $contextmenu)) this.show = false;
		});
		this.art.on("blur", () => {
			this.show = false;
		});
...
function getPosFromEvent(art, event) {
	...
	const width = clamp((isMobile ? event.touches[0].clientX : event.clientX) - left, 0, $progress.clientWidth);
	const second = width / $progress.clientWidth * art.duration;
	return {
		second,
		time: secondToTime(second),
		width,
		percentage: clamp(width / $progress.clientWidth, 0, 1)
	};
}
	...
	return (art) => {
		...
		return {
			...
			mounted: ($control) => {
				...
				if (icons.indicator) append($indicator, icons.indicator);
				else setStyle($indicator, "backgroundColor", "var(--art-theme)");
				function showHighlight(event) {
					const { width } = getPosFromEvent(art, event);
					const { text } = event.target.dataset;
					$tip.textContent = text;
					const tipWidth = $tip.clientWidth;
					if (width <= tipWidth / 2) setStyle($tip, "left", 0);
					else if (width > $control.clientWidth - tipWidth / 2) setStyle($tip, "left", `${$control.clientWidth - tipWidth}px`);
					else setStyle($tip, "left", `${width - tipWidth / 2}px`);
				}
				...
				function updateHighlight() {
					...
					for (let index = 0; index < option.highlight.length; index++) {
						...
						append($highlight, `<span data-text="${item.text}" data-time="${item.time}" style="left: ${left}%"></span>`);
					}
				}
				function setBar(type, percentage, event) {
					const isMobileDragging = type === "played" && event && isMobile;
					if (type === "loaded") setStyle($loaded, "width", `${percentage * 100}%`);
					if (type === "hover") {
						setStyle($hover, "width", `${percentage * 100}%`);
						if (includeFromEvent(event, $highlight)) showHighlight(event);
						else showTime(event);
						if (percentage === 0) removeClass($player, "art-progress-hover");
						else addClass($player, "art-progress-hover");
					}
					if (type === "played") {
					...
					if (isMobileDragging) {
						...
						showTime(event, {
							width: $control.clientWidth * percentage,
							time: secondToTime(percentage * art.duration)
						});
...
function volume$1(option) {
	return (art) => ({
		...
		mounted: ($control) => {
			...
			const $loaded = append(append($slider, `<div class="art-volume-handle"></div>`), `<div class="art-volume-loaded"></div>`);
			...
			if (isMobile) setStyle($panel, "display", "none");
			else {
				let isDragging = false;
...
var Control = class extends Component {
	...
	init() {
		...
		if (!option.isLive) this.add(progress({
			name: "progress",
			position: "top",
			index: 10
		}));
		...
		if (!option.isLive) this.add(time({
			name: "time",
			position: "left",
			index: 30
		}));
		if (option.quality.length) sleep().then(() => {
			this.art.quality = option.quality;
		});
		if (option.screenshot && !isMobile) this.add(screenshot$1({
			name: "screenshot",
			position: "right",
			index: 20
		}));
		if (option.setting) this.add(setting$1({
			name: "setting",
			position: "right",
			index: 30
		}));
		if (option.pip) this.add(pip$1({
			name: "pip",
			position: "right",
			index: 40
		}));
		if (option.airplay && window.WebKitPlaybackTargetAvailabilityEvent) this.add(airplay$1({
			name: "airplay",
			position: "right",
			index: 50
		}));
		if (option.fullscreenWeb) this.add(fullscreenWeb({
			name: "fullscreenWeb",
			position: "right",
			index: 60
		}));
		if (option.fullscreen) this.add(fullscreen({
			name: "fullscreen",
			position: "right",
			index: 70
		}));
	...
	check(target) {
		...
		for (let index = 0; index < target.$control_option.length; index++) {
			...
			if (item.default) inverseClass(item.$control_item, "art-current");
		}
	}
	selector(option, $ref, events) {
		...
		for (let index = 0; index < option.selector.length; index++) {
			...
			append($list, $item);
			def(item, "$control_option", { get: () => option.selector });
			def(item, "$control_item", { get: () => $item });
			def(item, "$control_value", { get: () => $value });
		}
		const event = proxy($list, "click", async (event2) => {
			const path = getComposedPath(event2);
			const item = option.selector.find((item2) => item2.$control_item === path.find(($item) => item2.$control_item === $item));
			this.check(item);
			if (option.onSelect) $value.innerHTML = await option.onSelect.call(this.art, item, item.$control_item, event2);
		});
		events.push(event);
	}
};
	...
	if (isMobile && !art.option.isLive) {
		...
		const onTouchStart = (event) => {
			if (event.touches.length === 1 && !art.isLock) {
				if (touchTarget === $progress) setCurrentTime(art, event);
				isDragging = true;
		...
		const onTouchMove = (event) => {
			if (event.touches.length === 1 && isDragging && art.duration) {
				...
				if (isHorizontal && !art.isRotate || isVertical && art.isRotate) {
...
function viewInit(art) {
	...
	art.on("view", (state2) => {
		if (option.autoMini) art.mini = !state2;
	});
}
var Events = class {
	constructor(art) {
...
var Icons = class {
	constructor(art) {
		...
		for (const key in icons) def(this, key, { get: () => getIcon(key, icons[key]) });
	}
};
var Info = class extends Component {
	constructor(art) {
		super(art);
		this.name = "info";
		if (!isMobile) this.init();
...
var Notice = class {
	...
	set show(msg) {
		...
		} else removeClass($player, "art-notice-show");
	}
	get show() {
...
function attrMix(art) {
	...
	def(art, "attr", { value(key, value) {
		if (value === void 0) return $video[key];
		$video[key] = value;
	} });
}
function autoHeightMix(art) {
	const { template: { $container, $video } } = art;
	def(art, "autoHeight", { value() {
...
function cssVarMix(art) {
	const { $player } = art.template;
	def(art, "cssVar", { value(key, value) {
...
function eventInit(art) {
	...
	let reconnectTime = 0;
	for (let index = 0; index < config$1.events.length; index++) proxy($video, config$1.events[index], (event) => {
...
function flipMix(art) {
	...
	def(art, "flip", {
		...
		set(flip2) {
			if (!flip2) flip2 = "normal";
			if (flip2 === "normal") delete $player.dataset.flip;
			else $player.dataset.flip = flip2;
...
var nativeAPI = (() => {
	...
	const returnValue = {};
	for (const methodList of methodMap) if (methodList[1] in document) {
...
Object.defineProperties(screenfull, {
	isFullscreen: { get: () => Boolean(document[nativeAPI.fullscreenElement]) },
	element: {
		enumerable: true,
...
function fullscreenMix(art) {
	...
	const nativeScreenfull = (art2) => {
		screenfull.on("change", () => {
			...
			} else removeClass($player, "art-fullscreen");
			art2.emit("resize");
		});
		...
		def(art2, "fullscreen", {
			...
			async set(value) {
				if (value) await screenfull.request($player);
				else await screenfull.exit();
			}
		});
	};
	...
	art.once("video:loadedmetadata", () => {
		if (screenfull.isEnabled) nativeScreenfull(art);
		else if ($video.webkitSupportsFullscreen) webkitScreenfull(art);
		else def(art, "fullscreen", {
			get() {
				return false;
			},
			set() {
	...
	def(art, "fullscreenWeb", {
		...
		set(value) {
			if (value) {
				...
				if (constructor.FULLSCREEN_WEB_IN_BODY) append(document.body, $player);
...
function loadedMix(art) {
	const { $video } = art.template;
	def(art, "loaded", { get: () => art.loadedTime / $video.duration });
	def(art, "loadedTime", { get: () => {
		...
		if ($mini) {
			...
			art.on("document:mousemove", (event) => {
				if (isDragging) {
					...
					setStyle($mini2, "transform", `translate(${event.pageX - lastPageX}px, ${event.pageY - lastPageY}px)`);
				}
			});
			art.on("document:mouseup", () => {
				if (isDragging) {
...
function optionInit(art) {
	...
	if (option.volume) $video.volume = clamp(option.volume, 0, 1);
	const volumeStorage = storage.get("volume");
	if (typeof volumeStorage === "number") $video.volume = clamp(volumeStorage, 0, 1);
	if (option.poster) setStyle($poster, "backgroundImage", `url(${option.poster})`);
...
function playMix(art) {
	const { i18n, notice, option, constructor: { instances: instances2 }, template: { $video } } = art;
	def(art, "play", { async value() {
		const result = await $video.play();
		notice.show = i18n.get("Play");
		art.emit("play");
		if (option.mutex) for (let index = 0; index < instances2.length; index++) {
...
function screenshotMix(art) {
	...
	def(art, "getDataURL", { value: () => new Promise((resolve, reject) => {
		try {
	...
	def(art, "getBlobUrl", { value: () => new Promise((resolve, reject) => {
		try {
			$canvas.width = $video.videoWidth;
			$canvas.height = $video.videoHeight;
			$canvas.getContext("2d").drawImage($video, 0, 0);
	...
	def(art, "screenshot", { value: async (name) => {
		const dataUri = await art.getDataURL();
		download(dataUri, `${name || `artplayer_${secondToTime($video.currentTime)}`}.png`);
...
function seekMix(art) {
	const { notice } = art;
	def(art, "seek", { set(time2) {
		art.currentTime = time2;
		if (art.duration) notice.show = `${secondToTime(art.currentTime)} / ${secondToTime(art.duration)}`;
		art.emit("seek", art.currentTime, time2);
	} });
	def(art, "forward", { set(time2) {
		art.seek = art.currentTime + time2;
	} });
	def(art, "backward", { set(time2) {
		art.seek = art.currentTime - time2;
	} });
}
...
function switchMix(art) {
	...
	def(art, "switchQuality", { value: (url) => {
		return switchUrl(url, art.currentTime);
	} });
	def(art, "switchUrl", { value: (url) => {
		return switchUrl(url, 0);
	} });
	def(art, "switch", { set: art.switchUrl });
}
...
function thumbnailsMix(art) {
	...
	function showThumbnails(posWidth) {
		...
		if (posWidth <= width2 / 2) setStyle($thumbnails, "left", 0);
		else if (posWidth > $progress.clientWidth - width2 / 2) setStyle($thumbnails, "left", `${$progress.clientWidth - width2}px`);
		else setStyle($thumbnails, "left", `${posWidth - width2 / 2}px`);
	}
	art.on("setBar", async (type, percentage, event) => {
		const $thumbnails = art.controls?.thumbnails;
		const { url, scale } = option.thumbnails;
		if (!$thumbnails || !url) return;
		if (type === "hover" || type === "played" && event && isMobile) {
			if (!image && !loding) {
				loding = true;
	...
	art.on("fullscreenWeb", (state2) => {
		if (state2) {
			if (needRotate()) {
				...
				setTimeout(() => {
					if (art.fullscreenWeb && !hasClass($player, WEB_CLASS)) applyWebRotate();
				}, delay);
			}
		} else if (hasClass($player, WEB_CLASS)) clearWebRotate();
	});
	art.on("fullscreen", async (state2) => {
		const canLock = !!screen?.orientation?.lock;
		if (state2) {
			if (canLock && needRotate()) try {
				const opposite = screen.orientation.type.startsWith("portrait") ? "landscape" : "portrait";
			...
			if (hasClass($player, FS_CLASS)) removeClass($player, FS_CLASS);
			if (canLock && fsLocked) {
				try {
...
function autoPlayback(art) {
	...
	function init() {
		const currentTime = (storage.get("times") || {})[art.option.id || art.option.url];
		clearTimeout(timer);
...
function miniProgressBar(art) {
	art.on("control", (state2) => {
		if (state2) removeClass(art.template.$player, "art-mini-progress-bar");
		else addClass(art.template.$player, "art-mini-progress-bar");
	});
...
var Plugins = class {
	constructor(art) {
		...
		this.id = 0;
		const { option } = art;
		if (option.miniProgressBar && !option.isLive) this.add(miniProgressBar);
		if (option.lock && isMobile) this.add(lock);
		if (option.autoPlayback && !option.isLive) this.add(autoPlayback);
		if (option.autoOrientation && isMobile) this.add(autoOrientation);
		if (option.fastForward && isMobile && !option.isLive) this.add(fastForward);
	...
	next(plugin, result) {
		...
		def(this, pluginName, { value: result });
		return this;
	}
};
...
var Setting = class extends Component {
	constructor(art) {
		super(art);
	...
	get builtin() {
		const result = [];
		const { option } = this.art;
		if (option.playbackRate) result.push(playbackRate(this.art));
		if (option.aspectRatio) result.push(aspectRatio(this.art));
		if (option.flip) result.push(flip(this.art));
		if (option.subtitleOffset) result.push(subtitleOffset(this.art));
		return result;
	}
	...
	check(target) {
		...
		this.traverse((item) => {
			item.default = item === target;
			if (item.default && item.$item) inverseClass(item.$item, "art-current");
		}, target.$option);
	...
	format(option = this.option, parent, parents, names = []) {
		for (let index = 0; index < option.length; index++) {
			...
			if (!item.$formatted) {
				def(item, "$parent", { get: () => parent });
				def(item, "$parents", { get: () => parents });
				def(item, "$option", { get: () => option });
				const $events = [];
				def(item, "$events", { get: () => $events });
				def(item, "$formatted", { get: () => true });
			}
	...
	resize() {
		...
		if (controls.setting && this.show) {
			...
			setStyle($setting, "height", `${this.active === this.option ? this.active.length * SETTING_ITEM_HEIGHT : (this.active.length + 1) * SETTING_ITEM_HEIGHT}px`);
	...
	createHeader(item) {
		...
		const { proxy, icons: { arrowLeft: arrowLeft2 }, constructor: { SETTING_ITEM_HEIGHT } } = this.art;
		const $item = createElement("div");
	...
	createItem(item, isUpdate = false) {
		...
		let type = "selector";
		if (has(item, "switch")) type = "switch";
		if (has(item, "range")) type = "range";
		if (has(item, "onClick")) type = "button";
		...
		switch (type) {
			...
			case "range":
				...
				break;
			case "selector":
				if (item.selector?.length) append($icon, item.icon || icons.config);
				else append($icon, icons.check);
				break;
		}
		append($left, $icon);
		def(item, "$icon", {
			...
			case "selector":
				{
					const event = proxy($item, "click", async (event2) => {
						if (item.selector?.length) this.render(item.selector);
							...
							this.check(item);
							if (item.$parent.onSelect) item.$parent.tooltip = await item.$parent.onSelect.call(this.art, item, $item, event2);
						}
					});
					item.$events.push(event);
					if (item.default) addClass($item, "art-current");
		...
		if (isUpdate) replaceElement($item, oldItem);
		else append($panel, $item);
		if (item.mounted) setTimeout(() => item.mounted.call(this.art, item.$item, item), 0);
	}
	render(option = this.option) {
		this.active = option;
		if (this.cache.has(option)) inverseClass(this.cache.get(option), "art-current");
		else {
...
var Storage = class {
	...
	set(key, value) {
		try {
			const storage = Object.assign({}, this.get(), { [key]: value });
...
var Subtitle = class extends Component {
	constructor(art) {
		...
		art.on("video:timeupdate", () => {
			...
			if (typeof state2 !== "boolean") return;
			if (state2 !== lastState) {
				lastState = state2;
	...
	style(key, value) {
		...
		if (typeof key === "object") return setStyles($subtitle, key);
	...
	async init(subtitleOption) {
		...
		return fetch(subtitleOption.url).then((response) => response.arrayBuffer()).then((buffer) => {
			const text = new TextDecoder(subtitleOption.encoding).decode(buffer);
			switch (subtitleOption.type || getExt(subtitleOption.url)) {
				case "srt": {
					const vtt = srtToVtt(text);
					return vttToBlob(subtitleOption.onVttLoad(vtt));
				}
				case "ass": {
					const vtt = assToVtt(text);
					return vttToBlob(subtitleOption.onVttLoad(vtt));
				}
				case "vtt": return vttToBlob(subtitleOption.onVttLoad(text));
				default: return subtitleOption.url;
			}
		}).then((subUrl) => {
			$subtitle.innerHTML = "";
...
var Template = class Template {
	constructor(art) {
		...
		errorHandle(constructor.instances.every((ins) => ins.template.$container !== this.$container), "Cannot mount multiple instances on the same dom element");
	...
	init() {
		...
		if (option.proxy) {
			...
			errorHandle(video instanceof HTMLVideoElement || video instanceof HTMLCanvasElement, `Function 'option.proxy' needs to return 'HTMLVideoElement' or 'HTMLCanvasElement'`);
			replaceElement(video, this.$video);
			video.className = "art-video";
			this.$video = video;
		}
		if (option.backdrop) addClass(this.$player, "art-backdrop");
		if (isMobile) addClass(this.$player, "art-mobile");
	}
	destroy(removeHtml) {
		if (removeHtml) this.$container.innerHTML = "";
		else addClass(this.$player, "art-destroy");
	}
};
var Emitter = class {
	on(name, fn, ctx) {
...
var id = 0;
var instances = [];
var Artplayer = class Artplayer extends Emitter {
	constructor(option, readyCallback) {
		super();
		if (!isBrowser) throw new Error("Artplayer can only be used in the browser environment");
		this.id = ++id;
		...
		if (Artplayer.DEBUG) {
			...
			for (let index = 0; index < config$1.events.length; index++) this.on(`video:${config$1.events[index]}`, (event) => log(`Event@${event.type}`));
		}
		instances.push(this);
	}
...
Artplayer.TOUCH_MOVE_RATIO = .5;
Artplayer.VOLUME_STEP = .1;
